export class FimsMap {
  ...
  bindMapEvents() {
    this.map.on("mousemove", ({ latlng }) => {
      this.onCoordinate(`Lat: ${latlng.lat.toFixed(5)} / Lon: ${latlng.lng.toFixed(5)}`);
    });
    this.map.on("mouseout", () => this.onCoordinate("Lat: — / Lon: —"));
  }
  ...
  getQualifiedLayerName(layer) {
    ...
    return layer.name.includes(":") ? layer.name : `${this.config.geoserver.workspace}:${layer.name}`;
  }
  ...
  setLayerVisible(key, visible) {
    ...
    if (!layer && visible) {
      ...
      if (!layer) {
        this.onStatus(`Layer '${key}' is not connected. Configure js/config.js.`);
        return false;
      }
    ...
    if (visible && layer && !this.map.hasLayer(layer)) {
      ...
      this.onStatus(`${this.getLayerDefinition(key)?.label || key} displayed`);
    } else if (!visible && layer && this.map.hasLayer(layer)) {
      this.map.removeLayer(layer);
      this.onStatus(`${this.getLayerDefinition(key)?.label || key} hidden`);
    }
    return true;
  }
