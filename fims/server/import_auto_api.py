...
def _column_names(connection: Any, table_name: str) -> list[str]:
...
def _copy_stage_to_target(
    ...
    source_columns = set(_column_names(connection, stage_table))
    target_columns = _column_names(connection, target)
    ...
    common_columns = [
        ...
        if column in source_columns and column not in ignored_columns
    ]
    ...
    connection.execute(
        text(
            f"""
            INSERT INTO public."{target}" ({quoted_columns})
